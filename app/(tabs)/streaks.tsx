  ...
  interface StreakData{
    ...
    habitCompletions?.forEach((c) => {
      ...
      if (lastDate) {
        ...
        if (diff <= 1.5){
          currentStreak += 1  // ← Tiếp tục streak liên tiếp
        } else {
          // ← SỬA LỖI 2: Check bestStreak TRƯỚC khi reset
          if (currentStreak > bestStreak) bestStreak = currentStreak; // ← THÊM: Save best streak trước reset
          currentStreak = 1  // ← Reset streak mới
        }
      } else {
        // ← Lần đầu tiên
        currentStreak = 1;
      }
      // ← XÓA: Dòng này nằm ngoài nên xóa else thứ 2

      lastDate = date; // ← Di chuyển vào cuối (sau tất cả check)
    })

    // ← SỬA LỖI 3: Check bestStreak cuối cùng (sau vòng lặp)
    if (currentStreak > bestStreak) bestStreak = currentStreak; // ← THÊM: Check lần cuối
    streak = currentStreak; // ← Set streak = currentStreak cuối cùng
